-module(rb_sequencer_paxos_state).
...
-record(state, {
    ...
    %% An aggregated clock of all causal dependencies of all prepared transactions
    aggregated_vc :: vclock(),
    index :: cache_id(),
...
-type ready_tx() :: [ red_heartbeat_id() | {term(), tx_label(), partition_ws(), vclock()}, ... ].
...
-spec new() -> t().
new() ->
    #state{aggregated_vc=pvc_vclock:new(),
...
-spec get_decided_data(record_id(), t()) -> {tx_label(), writeset(), vclock()}.
get_decided_data(Id, #state{entries=EntryMap}) ->
    #tx_data{label=Label, writeset=WS, clock=VC} = maps:get(Id, EntryMap),
    {Label, WS, VC}.
...
-spec data_for_id(record_id(), #{record_id() => tx_data()}) -> red_heartbeat_id() | {term(), tx_label(), partition_ws(), vclock()}.
data_for_id({?red_heartbeat_marker, Id}, _) -> {?red_heartbeat_marker, Id};
data_for_id(Id, EntryMap) ->
    #tx_data{label=Label, writeset=PWS, clock=CommitVC} = maps:get(Id, EntryMap),
    {Id, Label, PWS, CommitVC}.
...
accept_hb(Ballot, Id, Ts, State) ->
    accept(Ballot, Id, ignore, #{}, #{}, ok, Ts, State).
...
%% Merge all readset lists into one, also return the list of all partitions
%% where this transaction should be delivered
-spec expand_readset(partition_rs()) -> readset().
expand_readset(PRS) ->
    maps:fold(
        fun(_, RS, RSAcc) ->
            lists:foldl(
                fun(Key, Acc) -> [Key | Acc] end,
                RSAcc,
                RS
            )
        end,
...
%% Merge all writsets into one, also return the list of all keys
-spec expand_writeset(partition_ws()) -> {[key()], writeset()}.
expand_writeset(PWS) ->
    maps:fold(
        fun(_, WS, {AllKeys, WSAcc}) ->
            maps:fold(
                fun(Key, Value, {InKeyAcc, InWSAcc}) ->
                    {[Key | InKeyAcc], InWSAcc#{Key => Value}}
                end,
                {AllKeys, WSAcc},
                WS
            )
...
-spec partition_writeset_keys(partition_ws()) -> [key()].
partition_writeset_keys(PWS) ->
    maps:fold(
        fun(_, WS, AllKeys) ->
            maps:fold(
                fun(Key, _, InKeyAcc) ->
...
-spec prepare(Id :: record_id(),
              ...
              PWS :: partition_ws(),
              SnapshotVC :: vclock(),
              LastVC :: cache({key(), tx_label()}, vclock()),
              Conflicts :: conflict_relations(),
              State :: t()) -> {prepare_result(), t()}.

prepare(Id, Label, PRS, PWS, SnapshotVC, LastVC, Conflicts, State=#state{ballot=Ballot,
                                                                         entries=EntryMap,
                                                                         aggregated_vc=AggregatedVC0,
    ...
    case maps:get(Id, EntryMap, empty) of
        ...
        empty ->
            %% There are no blind writes (WS \subseteq RS), so we can take the partitions from the readset
            AllReadSet = expand_readset(PRS),
            {AllWSKeys, AllWriteSet} = expand_writeset(PWS),
            Decision = check_transaction(Label, AllReadSet, AllWriteSet, SnapshotVC,
                                         LastVC, PendingReads, PendingWrites, Conflicts),
            {RedTs, PrepareVC, AggregatedVC} = assign_timestamp(SnapshotVC, AggregatedVC0),
            ...
            {{Decision, Ballot, PrepareVC}, State#state{entries=EntryMap#{Id => Record}, aggregated_vc=AggregatedVC}}
    end.

-spec accept(ballot(), record_id(), tx_label(), partition_rs(), partition_ws(), red_vote(), vclock(), t()) -> {ok, t()} | bad_ballot.
accept(InBallot, _, _, _, _, _, _, #state{ballot=Ballot}) when InBallot =/= Ballot -> bad_ballot;
accept(Ballot, Id, Label, PRS, PWS, Vote, PrepareVC, S=#state{ballot=Ballot, entries=EntryMap}) ->
...
-dialyzer({no_opaque, assign_timestamp/2}).
assign_timestamp(ignore, Vector) ->
    Time = rb_sequencer_time:timestamp(),
    {Time, Time, grb_vclock:set_time(?RED_REPLICA, Time, Vector)};

assign_timestamp(VC, Aggregated) ->
    Max = grb_vclock:max(VC, Aggregated),
    Red = rb_sequencer_time:timestamp(),
    {Red, grb_vclock:set_time(?RED_REPLICA, Red, Max), Max}.
...
make_prepared(Id, Label, RS, PWS, WSKeys, RedTs, VC, Decision, #state{index=Idx,
                                                                      pending_reads=Reads,
                                                                      writes_cache=Writes}) ->
    ...
    case Decision of
        ok ->
            make_commit_record(Id, Label, RS, PWS, WSKeys, RedTs, VC, Decision, Reads, Writes);
        _ ->
            %% don't store read / written keys, waste of space
            #tx_data{label=Label, writeset=PWS, red_ts=RedTs, clock=VC, state=prepared, vote=Decision}
    end.

-spec make_commit_record(record_id(), tx_label(), readset(), partition_ws(), [key()], grb_time:ts(), _, red_vote(), cache_id(), cache_id()) -> tx_data().
make_commit_record({?red_heartbeat_marker, _}, _, _, _, _, RedTs, _, Decision, _, _) ->
    #tx_data{label=undefined, read_keys=[], write_keys=[], writeset=#{},
             red_ts=RedTs, clock=RedTs, state=prepared, vote=Decision};

make_commit_record(TxId, Label, RS, PWS, WriteKeys, RedTs, VC, Decision, Reads, Writes) ->
    ...
    #tx_data{label=Label, read_keys=RS, write_keys=WriteKeys, writeset=PWS,
...
-spec check_transaction(Label :: tx_label(),
                        RS :: readset(),
                        WS :: writeset(),
                        SnapshotVC :: vclock(),
                        LastVC :: cache({key(), tx_label()}, vclock()),
...
check_transaction(Label, RS, WS, SnapshotVC, LastVC, PrepReads, Writes, Conflicts)
...
check_transaction(_, _, _, _, _, _, _, _) ->
    ok.
...
-spec check_committed(tx_label(), readset(), vclock(), cache_id(), cache({key(), tx_label()}, vclock())) -> red_vote().
check_committed(ConflictLabel, RS, SnapshotVC, PendingWrites, LastVC) ->
    try
        lists:foreach(fun(Key) ->
            case stale_decided(Key, ConflictLabel, SnapshotVC, PendingWrites) of
                true ->
                    throw(?abort_stale_dec);
                false ->
                    case stale_committed(Key, ConflictLabel, SnapshotVC, LastVC) of
                        true ->
...
stale_decided(Key, ConflictLabel, PrepareVC, PendingWrites) ->
    ...
    case Match of
        ...
        Vectors ->
            lists:any(fun(CommitVC) ->
                not (grb_vclock:leq(CommitVC, PrepareVC))
            end, Vectors)
    end.

-spec stale_committed(key(), tx_label(), vclock(), cache({key(), tx_label()}, vclock())) -> boolean().
stale_committed(Key, ConflictLabel, PrepareVC, LastVC) ->
    case ets:lookup(LastVC, {Key, ConflictLabel}) of
        [{_, LastCommitVC}] ->
            not (grb_vclock:leq(LastCommitVC, PrepareVC));
        [] ->
            false
    end.
...
-spec with_states(rb_sequencer_paxos_state:t() | [rb_sequencer_paxos_state:t()],
                  fun((rb_sequencer_paxos_state:t() | [rb_sequencer_paxos_state:t()]) -> _)) -> ok.
...
rb_sequencer_paxos_state_heartbeat_test() ->
    with_states([rb_sequencer_paxos_state:new(),
                 rb_sequencer_paxos_state:new()], fun([L0, F0]) ->

        Id = {?red_heartbeat_marker, 0},
        {{ok, Ballot, Ts}, L1} = rb_sequencer_paxos_state:prepare_hb(Id, L0),
        {ok, F1} = rb_sequencer_paxos_state:accept_hb(Ballot, Id, Ts, F0),

        {ok, L2} = rb_sequencer_paxos_state:decision_hb(Ballot, Id, Ts, L1),
        {ok, F2} = rb_sequencer_paxos_state:decision_hb(Ballot, Id, Ts, F1),

        {Ts, _} = rb_sequencer_paxos_state:get_next_ready(0, L2),
        ?assertMatch({Ts, [Id]}, rb_sequencer_paxos_state:get_next_ready(0, F2)),
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(Ts, L2)),
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(Ts, F2))
    end).

rb_sequencer_paxos_state_transaction_test() ->
    ok = persistent_term:put({rb_sequencer_dc_manager, my_replica}, ignore),
    ...
    with_states([rb_sequencer_paxos_state:new(),
...

        {
            Res={ok, Ballot, CVC},
            L1
        } = rb_sequencer_paxos_state:prepare(TxId, Label, RS, WS, PVC, LastVC, Conflicts, L0),

        %% preparing again should result in the same result
        {OtherRes, L2} = rb_sequencer_paxos_state:prepare(TxId, Label, RS, WS, PVC, LastVC, Conflicts, L1),
        ?assertEqual(Res, OtherRes),
        ?assertEqual(L1, L2),

        {ok, F1} = rb_sequencer_paxos_state:accept(Ballot, TxId, Label, RS, WS, ok, CVC, F0),

        {ok, L3} = rb_sequencer_paxos_state:decision(Ballot, TxId, ok, grb_vclock:get_time(?RED_REPLICA, CVC), L2),
        {ok, F2} = rb_sequencer_paxos_state:decision(Ballot, TxId, ok, grb_vclock:get_time(?RED_REPLICA, CVC), F1),

        %% preparing after decision should return early
        {PrepRes, L4} = rb_sequencer_paxos_state:prepare(TxId, Label, RS, WS, PVC, LastVC, Conflicts, L3),
        ?assertEqual({already_decided, ok, CVC} , PrepRes),
        ?assertEqual(L3 , L4),

        Expected = grb_vclock:get_time(?RED_REPLICA, CVC),
        Ready={CommitTime, [{TxId, Label, WS, Clock}]} = rb_sequencer_paxos_state:get_next_ready(0, L4),
        ?assertEqual(Ready, rb_sequencer_paxos_state:get_next_ready(0, F2)),
        ?assertEqual(Expected, CommitTime),
        ?assertEqual(CVC, Clock),

        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(CommitTime, L4)),
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(CommitTime, F2))
    end),
    true = ets:delete(LastVC),
    true = persistent_term:erase({rb_sequencer_dc_manager, my_replica}).

rb_sequencer_paxos_state_get_next_ready_test() ->
    ...
    with_states(rb_sequencer_paxos_state:new(), fun(F0) ->
        [V0, V1, V2] = [ VC(1), VC(5), VC(7)],
        {ok, F1} = rb_sequencer_paxos_state:accept(0, tx_0, <<>>, #{}, #{}, ok, V0, F0),
        {ok, F2} = rb_sequencer_paxos_state:accept(0, tx_1, <<>>, #{}, #{}, ok, V1, F1),
        {ok, F3} = rb_sequencer_paxos_state:accept(0, tx_2, <<>>, #{}, #{}, ok, V2, F2),

        {ok, F4} = rb_sequencer_paxos_state:decision(0, tx_1, ok, grb_vclock:get_time(?RED_REPLICA, V1), F3),
        {ok, F5} = rb_sequencer_paxos_state:decision(0, tx_2, ok, grb_vclock:get_time(?RED_REPLICA, V2), F4),

        %% false, still have to decide tx_0, and it was prepared with a lower time
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(0, F5)),
        %% now decide tx_0, marking tx_1 ready for delivery
        {ok, F6} = rb_sequencer_paxos_state:decision(0, tx_0, ok, grb_vclock:get_time(?RED_REPLICA, V0), F5),

        ?assertEqual({T(V0), [{tx_0, <<>>, #{}, V0}]}, rb_sequencer_paxos_state:get_next_ready(0, F6)),
        ?assertEqual({T(V1), [{tx_1, <<>>, #{}, V1}]}, rb_sequencer_paxos_state:get_next_ready(T(V0), F6)),
        ?assertEqual({T(V2), [{tx_2, <<>>, #{}, V2}]}, rb_sequencer_paxos_state:get_next_ready(T(V1), F6))
    end),
    ...
    with_states(rb_sequencer_paxos_state:new(), fun(F0) ->
        ...
        {ok, F1} = rb_sequencer_paxos_state:accept_hb(0, {?red_heartbeat_marker, 0}, T0, F0),
        {ok, F2} = rb_sequencer_paxos_state:accept(0, tx_1, <<>>, #{}, #{}, ok, V1, F1),

        {ok, F3} = rb_sequencer_paxos_state:decision(0, tx_1, ok, grb_vclock:get_time(?RED_REPLICA, V1), F2),

        %% false, still have to decide the heartbeat
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(0, F3)),

        %% now decide the heartbeat, marking tx_1 ready for delivery
        {ok, F4} = rb_sequencer_paxos_state:decision_hb(0, {?red_heartbeat_marker, 0}, T0, F3),

        ?assertEqual({T0, [{?red_heartbeat_marker, 0}]}, rb_sequencer_paxos_state:get_next_ready(0, F4)),
        ?assertEqual({T(V1), [{tx_1, <<>>, #{}, V1}]}, rb_sequencer_paxos_state:get_next_ready(T0, F4))
    end),
    ...
    with_states(rb_sequencer_paxos_state:new(), fun(F0) ->
        ...
        {ok, F1} = rb_sequencer_paxos_state:accept(0, tx_0, <<>>, #{}, #{}, {abort, ignore}, V0, F0),
        {ok, F2} = rb_sequencer_paxos_state:accept(0, tx_1, <<>>, #{}, #{}, ok, V1, F1),
        {ok, F3} = rb_sequencer_paxos_state:accept(0, tx_2, <<>>, #{}, #{}, ok, V2, F2),
        {ok, F4} = rb_sequencer_paxos_state:accept(0, tx_3, <<>>, #{}, #{}, ok, V3, F3),

        {ok, F5} = rb_sequencer_paxos_state:decision(0, tx_1, ok, grb_vclock:get_time(?RED_REPLICA, V1), F4),

        %% we can skip tx_0, since it was aborted, no need to wait for decision (because decision will be abort)
        ?assertEqual({T(V1), [{tx_1, <<>>, #{}, V1}]}, rb_sequencer_paxos_state:get_next_ready(0, F5)),

        %% now we decide tx_2 with a higher time, placing it the last on the queue
        {ok, F6} = rb_sequencer_paxos_state:decision(0, tx_2, ok, grb_vclock:get_time(?RED_REPLICA, V4), F5),

        %% and decide tx_3, making it ready for delivery
        {ok, F7} = rb_sequencer_paxos_state:decision(0, tx_3, ok, grb_vclock:get_time(?RED_REPLICA, V3), F6),

        ?assertEqual({T(V3), [{tx_3, <<>>, #{}, V3}]}, rb_sequencer_paxos_state:get_next_ready(T(V1), F7)),
        ?assertEqual({T(V4), [{tx_2, <<>>, #{}, V4}]}, rb_sequencer_paxos_state:get_next_ready(T(V3), F7))
    end).

rb_sequencer_paxos_state_hb_clash_test() ->
    with_states(rb_sequencer_paxos_state:new(), fun(L0) ->
        {{ok, B, Ts0}, L1} = rb_sequencer_paxos_state:prepare_hb({?red_heartbeat_marker, 0}, L0),
        {ok, L2} = rb_sequencer_paxos_state:decision_hb(B, {?red_heartbeat_marker, 0}, Ts0, L1),

        ?assertEqual({Ts0, [{?red_heartbeat_marker, 0}]}, rb_sequencer_paxos_state:get_next_ready(0, L2)),
        {PrepRes, L3} = rb_sequencer_paxos_state:prepare_hb({?red_heartbeat_marker, 0}, L2),
        ?assertMatch({already_decided, ok, Ts0}, PrepRes),

        ?assertEqual({Ts0, [{?red_heartbeat_marker, 0}]}, rb_sequencer_paxos_state:get_next_ready(0, L3)),
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(Ts0, L3))
    end).

rb_sequencer_paxos_state_tx_clash_test() ->
    VC = fun(I) -> #{?RED_REPLICA => I} end,

    with_states(rb_sequencer_paxos_state:new(), fun(F0) ->
        {ok, F1} = rb_sequencer_paxos_state:accept(0, tx_1, <<>>, #{p => []}, #{p => #{a => 0}}, ok, VC(0), F0),
        {ok, F2} = rb_sequencer_paxos_state:accept(0, tx_2, <<>>, #{p => []}, #{p => #{a => 1}}, ok, VC(5), F1),

        {ok, F3} = rb_sequencer_paxos_state:decision(0, tx_1, ok, 5, F2),

        %% If we allow to deliver tx_1, we would overtake tx_2 and prevent it from ever being delivered
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(0, F3) ),

        {ok, F4} = rb_sequencer_paxos_state:decision(0, tx_2, ok, 5, F3),

        %% Now we can deliver, since tx_2 moved out of the prepare queue
        %% Order is not guaranteed, so sort both.
        {CommitTime, Ready} = rb_sequencer_paxos_state:get_next_ready(0, F4),
        ?assertEqual(5, CommitTime),
        ?assertEqual(lists:sort([ {tx_1, <<>>, #{p => #{a => 0}}, VC(5)},
                                  {tx_2, <<>>, #{p => #{a => 1}}, VC(5)} ]), lists:sort(Ready) )
    end).

rb_sequencer_paxos_state_check_prepared_sym_test() ->
...
rb_sequencer_paxos_state_check_prepared_asym_test() ->
    PendingReads = ets:new(dummy_reads, [bag]),
    PendingWrites =  ets:new(dummy_writes, [ordered_set]),
    Label1 = <<"a">>,
    Label2 = <<"b">>,
...
rb_sequencer_paxos_state_check_committed_sym_test() ->
    LastVC = ets:new(dummy_vc, [set]),
    PendingWrites =  ets:new(dummy_writes, [ordered_set]),
    ok = persistent_term:put({rb_sequencer_dc_manager, my_replica}, undefined),
    Label = <<>>,
...
rb_sequencer_paxos_state_check_committed_asym_test() ->
    LastVC = ets:new(dummy_vc, [set]),
    PendingWrites =  ets:new(dummy_writes, [ordered_set]),
    ok = persistent_term:put({rb_sequencer_dc_manager, my_replica}, undefined),
    Label1 = <<"a">>,
...
rb_sequencer_paxos_state_prune_decided_before_test() ->
    ok = persistent_term:put({rb_sequencer_dc_manager, my_replica}, ignore),
    LastVC = ets:new(dummy, [set]),
    ...
    with_states(rb_sequencer_paxos_state:new(), fun(L0) ->
        ...
        TxId1 = tx_1, Label1 = <<>>,
        RS1 = #{p => [a]}, WS1 = #{p => #{a => <<"hello">>}},
        PVC1 = #{?RED_REPLICA => 5}, Conflicts = #{},

        {{ok, Ballot, CVC1}, L1} = rb_sequencer_paxos_state:prepare(TxId1, Label1, RS1, WS1, PVC1, LastVC, Conflicts, L0),
        {ok, L2} = rb_sequencer_paxos_state:decision(Ballot, TxId1, ok, grb_vclock:get_time(?RED_REPLICA, CVC1), L1),

        %% preparing after decision should return early
        {PrepRes0, L3} = rb_sequencer_paxos_state:prepare(TxId1, Label1, RS1, WS1, PVC1, LastVC, Conflicts, L2),
        ?assertEqual({already_decided, ok, CVC1}, PrepRes0),
        ?assertEqual(L2, L3),

        Expected = grb_vclock:get_time(?RED_REPLICA, CVC1),
        ?assertEqual({Expected, [{TxId1, Label1, WS1, CVC1}]}, rb_sequencer_paxos_state:get_next_ready(0, L3)),
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(Expected, L3)),

        %% preparing even after delivery should return early
        {PrepRes1, L4} = rb_sequencer_paxos_state:prepare(TxId1, Label1, RS1, WS1, PVC1, LastVC, Conflicts, L3),
        ?assertEqual({already_decided, ok, CVC1}, PrepRes1),
        ?assertEqual(L3, L4),

        %% let's do a heartbeat now
        {{ok, Ballot, Ts}, L5} = rb_sequencer_paxos_state:prepare_hb({?red_heartbeat_marker, 0}, L4),
        {ok, L6} = rb_sequencer_paxos_state:decision_hb(Ballot, {?red_heartbeat_marker, 0}, Ts, L5),

        ?assertEqual({Ts, [{?red_heartbeat_marker, 0}]}, rb_sequencer_paxos_state:get_next_ready(Expected, L5)),
        ?assertEqual(false, rb_sequencer_paxos_state:get_next_ready(Ts, L5)),

        %% prune, now we should only have the heartbeat
        L7 = rb_sequencer_paxos_state:prune_decided_before(Expected + 1, L6),
        ?assertMatch({Ts, [{?red_heartbeat_marker, 0}]}, rb_sequencer_paxos_state:get_next_ready(0, L7)),

        %% but if we prune past that, then the heartbeat should be gone
        L8 = rb_sequencer_paxos_state:prune_decided_before(Ts + 1, L7),
        ?assertMatch(false, rb_sequencer_paxos_state:get_next_ready(0, L6)),

        %% now we can prepare this transaction again
        {PrepRes2, _} = rb_sequencer_paxos_state:prepare(TxId1, Label1, RS1, WS1, PVC1, LastVC, Conflicts, L8),
